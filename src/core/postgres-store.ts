...
export class PostgresStore extends MemoryStore {
  ...
  private async load(): Promise<void> {
    ...
    try {
      const [projects, sources, refreshRuns, artifacts, knowledge, provenance, deployments, snapshots, snapshotItems, movements, conflicts, auditEvents] = await Promise.all([
        client.query<Row>('SELECT id, slug, name, description, status, created_at AS "createdAt", updated_at AS "updatedAt" FROM projects ORDER BY created_at'),
        client.query<Row>('SELECT id, project_id AS "projectId", type, provider, config_json_encrypted AS "encryptedConfig", enabled, last_health_status AS "lastHealth", last_checked_at AS "lastCheckedAt" FROM project_sources ORDER BY id'),
        client.query<Row>('SELECT id, project_id AS "projectId", started_at AS "startedAt", completed_at AS "completedAt", status, source_revision_json AS "sourceRevisions", errors_json AS errors FROM refresh_runs ORDER BY started_at'),
        client.query<Row>('SELECT id, project_id AS "projectId", source_id AS "sourceId", external_id AS "externalId", path, artifact_type AS "artifactType", revision, content_hash AS "contentHash", content_text AS content, metadata_json AS metadata, first_seen_at AS "firstSeenAt", last_seen_at AS "lastSeenAt" FROM source_artifacts ORDER BY first_seen_at'),
        client.query<Row>('SELECT id, project_id AS "projectId", type, domain, title, body, status, documented_state AS documented, implemented_state AS implemented, tested_state AS tested, deployed_state AS deployed, observed_state AS observed, fingerprint, entity_key AS "entityKey", valid_from_snapshot_id AS "validFromSnapshotId", valid_to_snapshot_id AS "validToSnapshotId", created_at AS "createdAt" FROM knowledge_items ORDER BY created_at'),
        client.query<Row>('SELECT id, knowledge_item_id AS "knowledgeItemId", source_artifact_id AS "sourceArtifactId", source_type AS "sourceType", source_ref AS "sourceRef", repository_commit AS "repositoryCommit", path, start_line AS "startLine", end_line AS "endLine", metadata_json AS metadata FROM provenance ORDER BY id'),
        client.query<Row>('SELECT id, project_id AS "projectId", source_id AS "sourceId", external_id AS "externalId", environment, revision, status, deployed_at AS "deployedAt", metadata_json AS metadata FROM deployments ORDER BY deployed_at'),
        client.query<Row>('SELECT id, project_id AS "projectId", created_at AS "createdAt", repository_revision AS "repositoryRevision", deployment_revision AS "deploymentRevision", source_health_json AS "sourceHealth", summary_json AS summary FROM snapshots ORDER BY created_at'),
        client.query<Row>('SELECT snapshot_id AS "snapshotId", knowledge_item_id AS "knowledgeItemId" FROM snapshot_items ORDER BY snapshot_id'),
        client.query<Row>('SELECT id, project_id AS "projectId", from_snapshot_id AS "fromSnapshotId", to_snapshot_id AS "toSnapshotId", movement_type AS "movementType", entity_type AS "entityType", entity_key AS "entityKey", before_json AS before, after_json AS after, created_at AS "createdAt" FROM movements ORDER BY created_at'),
        client.query<Row>('SELECT id, project_id AS "projectId", snapshot_id AS "snapshotId", type, severity, title, description, evidence_json AS evidence, status FROM conflicts ORDER BY id'),
        client.query<Row>('SELECT id, actor_id AS "actorId", project_id AS "projectId", action, metadata_json AS metadata, created_at AS "createdAt" FROM audit_events ORDER BY created_at'),
      ]);
